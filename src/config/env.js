...
export const validateEnv = () => {
    ...
    if (!['development', 'production', 'test'].includes(process.env.NODE_ENV || 'development')) {
        logger.warn(`Invalid NODE_ENV: ${process.env.NODE_ENV}. Using 'development'`);
    }

    if (process.env.NODE_ENV === 'production' && 
        process.env.JWT_SECRET === 'your-super-secret-jwt-key-change-this-in-production-12345!@#') {
        logger.error('⚠️  CRITICAL: Using default JWT_SECRET in production! Change JWT_SECRET immediately!');
